package com.nomadpi.android

import android.net.Uri
import org.json.JSONArray
import org.json.JSONObject
import java.io.BufferedReader
import java.io.InputStreamReader
import java.net.HttpURLConnection
import java.net.URL
import java.net.URLEncoder
import java.nio.charset.StandardCharsets


data class UniversalRelease(
    val name: String,
    val infoHash: String,
    val quality: String,
    val size: String,
    val source: String,
    val codec: String,
    val details: String,
    val cached: Boolean,
    val compatible: Boolean,
    val directCandidate: Boolean,
    val reasons: List<String>,
)

data class UniversalTitle(
    val imdbId: String,
    val title: String,
    val year: String,
    val type: String,
    val poster: String?,
    val releases: List<UniversalRelease>,
)

data class ResolvedRemote(
    val title: String,
    val filename: String,
    val url: String,
    val proxyToken: String? = null,
)

data class PhotoAlbum(val name: String, val count: Int)

data class PhotoAlbums(
    val albums: List<PhotoAlbum>,
    val itemAlbums: Map<String, String>,
)

data class ComicBook(val title: String, val pages: List<String>)

/** Extra native-client API surface kept separate from the minimal core client. */
class NomadExtrasApi(private val api: NomadApi) {
    fun universalSearch(query: String, season: Int = 1, episode: Int = 1): List<UniversalTitle> {
        val q = enc(query.trim())
        val json = request("GET", "/api/debrid/universal/search?q=$q&season=$season&episode=$episode")
        val titles = json.optJSONArray("titles") ?: return emptyList()
        return buildList {
            for (i in 0 until titles.length()) {
                val title = titles.optJSONObject(i) ?: continue
                add(parseTitle(title))
            }
        }
    }

    fun universalReleases(title: UniversalTitle, season: Int = 1, episode: Int = 1, showAll: Boolean = false): UniversalTitle {
        val path = buildString {
            append("/api/debrid/universal/releases?imdb_id=")
            append(enc(title.imdbId))
            append("&media_type=").append(enc(title.type))
            append("&season=").append(season.coerceAtLeast(1))
            append("&episode=").append(episode.coerceAtLeast(1))
            append("&include_heavy=").append(if (showAll) "true" else "false")
            append("&limit=").append(if (showAll) 24 else 10)
        }
        val json = request("GET", path)
        return title.copy(releases = parseReleases(json.optJSONArray("releases")))
    }

    /** Resolve a cached release and create Nomad's authenticated Range proxy. */
    fun resolveForPlay(title: UniversalTitle, release: UniversalRelease, season: Int = 1, episode: Int = 1): ResolvedRemote {
        require(release.infoHash.isNotBlank()) { "Release has no info hash" }
        val magnet = request(
            "POST", "/api/debrid/magnet",
            JSONObject()
                .put("info_hash", release.infoHash)
                .put("title", title.title)
                .put("year", title.year)
                .put("media_type", title.type)
                .put("season", season.coerceAtLeast(1))
                .put("episode", episode.coerceAtLeast(1)),
        )
        val files = magnet.optJSONArray("files") ?: JSONArray()
        val links = magnet.optJSONArray("links") ?: JSONArray()
        val chosen = chooseVideo(files)
        val linkIndex = chosen.first.coerceIn(0, (links.length() - 1).coerceAtLeast(0))
        val link = links.optString(linkIndex).ifBlank { links.optString(0) }
        if (link.isBlank()) throw NomadApiException("Provider returned no playable video link")
        val unrestricted = request("POST", "/api/debrid/unrestrict", JSONObject().put("link", link))
        val remote = unrestricted.optString("url")
        if (remote.isBlank()) throw NomadApiException("Provider did not return a media URL")
        val raw = chosen.second.ifBlank { unrestricted.optString("filename", release.name) }
        val filename = raw.substringAfterLast('/').substringAfterLast('\\').ifBlank { "${title.title}.mp4" }
        val proxy = request(
            "POST", "/api/debrid/universal/play",
            JSONObject()
                .put("url", remote)
                .put("filename", filename)
                .put("mime_type", unrestricted.optString("mimeType").takeIf { it.isNotBlank() }),
        )
        val token = proxy.optString("token").takeIf { it.isNotBlank() }
        val url = proxy.optJSONObject("playback")?.optString("url").orEmpty()
        if (url.isBlank()) throw NomadApiException("Nomad did not create a remote playback URL")
        return ResolvedRemote(title.title, filename, api.absoluteUrl(url), token)
    }

    fun deleteRemotePlay(token: String) {
        request("DELETE", "/api/debrid/universal/play/${seg(token)}")
    }

    /** Resolve the release and enqueue a server-side Nomad download. */
    fun downloadRelease(title: UniversalTitle, release: UniversalRelease, season: Int = 1, episode: Int = 1): String {
        val resolved = resolveDirectSource(title, release, season, episode)
        val clean = request(
            "POST", "/api/debrid/clean-filename",
            JSONObject()
                .put("filename", resolved.filename)
                .put("title", title.title)
                .put("year", title.year)
                .put("media_type", title.type)
                .put("season", season)
                .put("episode", episode),
        ).optString("clean_filename", resolved.filename)
        val response = request(
            "POST", "/api/debrid/download",
            JSONObject()
                .put("url", resolved.url)
                .put("filename", clean)
                .put("category", if (title.type == "series") "shows" else "movies")
                .put("is_show", title.type == "series"),
        )
        return response.optString("download_id")
    }

    fun streamKeep(title: UniversalTitle, release: UniversalRelease, capabilities: AndroidCapabilities, season: Int = 1, episode: Int = 1): ResolvedRemote {
        val source = resolveDirectSource(title, release, season, episode)
        val body = JSONObject()
            .put("url", source.url)
            .put("filename", source.filename)
            .put("provider", "debrid")
            .put("category", if (title.type == "series") "shows" else "movies")
            .put("is_show", title.type == "series")
            .put("position", 0)
            .put("capabilities", capabilities.toJson())
            .put("metadata", JSONObject().put("title", title.title).put("season", season).put("episode", episode))
        val json = request("POST", "/api/playback/stream-keep/start", body)
        val playback = json.optJSONObject("playback") ?: JSONObject()
        val job = json.optJSONObject("job") ?: JSONObject()
        val url = playback.optString("url")
        if (url.isBlank()) throw NomadApiException("Stream + Keep did not return a playback URL")
        return ResolvedRemote(title.title, source.filename, api.absoluteUrl(url), job.optString("id").takeIf { it.isNotBlank() })
    }

    fun albums(): PhotoAlbums {
        val json = request("GET", "/api/playback/gallery/albums")
        val albums = json.optJSONArray("albums") ?: JSONArray()
        val out = buildList {
            for (i in 0 until albums.length()) {
                val item = albums.optJSONObject(i) ?: continue
                add(PhotoAlbum(item.optString("name"), item.optInt("count", 0)))
            }
        }
        val mapping = mutableMapOf<String, String>()
        val mapJson = json.optJSONObject("item_albums") ?: JSONObject()
        mapJson.keys().forEach { key -> mapping[key] = mapJson.optString(key) }
        return PhotoAlbums(out, mapping)
    }

    fun createAlbum(name: String) {
        request("POST", "/api/playback/gallery/albums", JSONObject().put("name", name.trim()))
    }

    fun movePhotos(ids: Collection<String>, album: String) {
        request(
            "POST", "/api/playback/gallery/move",
            JSONObject().put("item_ids", JSONArray(ids.toList())).put("album", album),
        )
    }

    fun deletePhotos(ids: Collection<String>): Int {
        return request(
            "POST", "/api/playback/gallery/bulk-delete",
            JSONObject().put("item_ids", JSONArray(ids.toList())),
        ).optInt("deleted", 0)
    }

    fun comic(path: String): ComicBook {
        val json = request("GET", "/api/media/books/comic/pages?path=${enc(path)}")
        val raw = json.optJSONArray("pages") ?: JSONArray()
        val pages = buildList {
            for (i in 0 until raw.length()) {
                val item = raw.opt(i)
                when (item) {
                    is String -> if (item.isNotBlank()) add(item)
                    is JSONObject -> item.optString("path").takeIf { it.isNotBlank() }?.let(::add)
                }
            }
        }
        return ComicBook(json.optString("title", path.substringAfterLast('/')), pages)
    }

    fun saveReaderProgress(path: String, page: Int, total: Int) {
        val percent = if (total > 0) ((page + 1).toDouble() / total * 100.0).coerceIn(0.0, 100.0) else 0.0
        request(
            "POST", "/api/playback/reader/progress",
            JSONObject()
                .put("path", path)
                .put("position", JSONObject().put("page", page).put("total", total))
                .put("percent", percent),
        )
    }

    fun readerPage(path: String): Int {
        val json = request("GET", "/api/playback/reader/progress?path=${enc(path)}")
        return json.optJSONObject("progress")?.optJSONObject("position")?.optInt("page", 0) ?: 0
    }

    private fun resolveDirectSource(title: UniversalTitle, release: UniversalRelease, season: Int, episode: Int): ResolvedRemote {
        val magnet = request(
            "POST", "/api/debrid/magnet",
            JSONObject()
                .put("info_hash", release.infoHash)
                .put("title", title.title)
                .put("year", title.year)
                .put("media_type", title.type)
                .put("season", season.coerceAtLeast(1))
                .put("episode", episode.coerceAtLeast(1)),
        )
        val files = magnet.optJSONArray("files") ?: JSONArray()
        val links = magnet.optJSONArray("links") ?: JSONArray()
        val chosen = chooseVideo(files)
        val index = chosen.first.coerceIn(0, (links.length() - 1).coerceAtLeast(0))
        val link = links.optString(index).ifBlank { links.optString(0) }
        if (link.isBlank()) throw NomadApiException("Provider returned no media link")
        val unrestricted = request("POST", "/api/debrid/unrestrict", JSONObject().put("link", link))
        val remote = unrestricted.optString("url")
        if (remote.isBlank()) throw NomadApiException("Provider did not return a media URL")
        val filename = chosen.second.ifBlank { unrestricted.optString("filename", release.name) }
            .substringAfterLast('/').substringAfterLast('\\')
        return ResolvedRemote(title.title, filename.ifBlank { "media.mp4" }, remote)
    }

    private fun chooseVideo(files: JSONArray): Pair<Int, String> {
        val video = Regex("\\.(mp4|mkv|m4v|webm|avi|mov|ts|m2ts|mts|wmv|mpg|mpeg)$", RegexOption.IGNORE_CASE)
        var bestIndex = 0
        var bestName = ""
        var bestSize = -1L
        var selectedOrdinal = 0
        for (i in 0 until files.length()) {
            val file = files.optJSONObject(i) ?: continue
            if (file.has("selected") && !file.optBoolean("selected", true)) continue
            val name = file.optString("path", file.optString("name"))
            val size = file.optLong("bytes", file.optLong("size", 0))
            if (video.containsMatchIn(name) && size >= bestSize) {
                bestSize = size
                bestIndex = selectedOrdinal
                bestName = name
            }
            selectedOrdinal++
        }
        return bestIndex to bestName
    }

    private fun parseTitle(json: JSONObject): UniversalTitle {
        val set = json.optJSONObject("release_set")
        return UniversalTitle(
            imdbId = json.optString("imdb_id"),
            title = json.optString("title", "Title"),
            year = json.optString("year"),
            type = json.optString("type", "movie"),
            poster = json.optString("poster").takeIf { it.isNotBlank() && it != "null" },
            releases = parseReleases(set?.optJSONArray("releases")),
        )
    }

    private fun parseReleases(array: JSONArray?): List<UniversalRelease> {
        if (array == null) return emptyList()
        return buildList {
            for (i in 0 until array.length()) {
                val item = array.optJSONObject(i) ?: continue
                val reasons = buildList {
                    val source = item.optJSONArray("lite_reasons")
                    if (source != null) for (j in 0 until source.length()) source.optString(j).takeIf { it.isNotBlank() }?.let(::add)
                }
                add(
                    UniversalRelease(
                        name = item.optString("name", "Release"),
                        infoHash = item.optString("info_hash", item.optString("hash")),
                        quality = item.optString("quality"),
                        size = item.optString("size"),
                        source = item.optString("source"),
                        codec = item.optString("codec"),
                        details = item.optString("details"),
                        cached = item.optBoolean("cached", false),
                        compatible = item.optBoolean("lite_compatible", false),
                        directCandidate = item.optBoolean("lite_direct_candidate", false),
                        reasons = reasons,
                    )
                )
            }
        }
    }

    private fun request(method: String, path: String, body: JSONObject? = null): JSONObject {
        val connection = (URL(api.absoluteUrl(path)).openConnection() as HttpURLConnection).apply {
            requestMethod = method
            connectTimeout = 10_000
            readTimeout = 60_000
            useCaches = false
            setRequestProperty("Accept", "application/json")
            setRequestProperty("User-Agent", "NomadAndroid/${BuildConfig.VERSION_NAME}")
            api.token?.takeIf { it.isNotBlank() }?.let { setRequestProperty("Authorization", "Bearer $it") }
            api.profileId?.let { setRequestProperty("X-Nomad-Profile-ID", it.toString()) }
            if (body != null) {
                doOutput = true
                setRequestProperty("Content-Type", "application/json; charset=utf-8")
                outputStream.use { it.write(body.toString().toByteArray(StandardCharsets.UTF_8)) }
            }
        }
        val status = try { connection.responseCode } catch (e: Exception) {
            connection.disconnect()
            throw NomadApiException("Could not reach ${api.baseUrl}: ${e.message ?: "network error"}")
        }
        val stream = if (status in 200..299) connection.inputStream else connection.errorStream
        val text = stream?.use { BufferedReader(InputStreamReader(it, StandardCharsets.UTF_8)).readText() }.orEmpty()
        connection.disconnect()
        val json = if (text.isBlank()) JSONObject() else runCatching { JSONObject(text) }.getOrElse { JSONObject().put("raw", text) }
        if (status !in 200..299) throw NomadApiException(errorText(json, status), status)
        return json
    }

    private fun errorText(json: JSONObject, status: Int): String {
        val detail = json.opt("detail")
        return when (detail) {
            is String -> detail
            is JSONObject -> detail.optString("message", detail.toString())
            is JSONArray -> (0 until detail.length()).joinToString("; ") { i ->
                val value = detail.opt(i)
                if (value is JSONObject) value.optString("msg", value.toString()) else value.toString()
            }
            else -> json.optString("message", "Nomad request failed ($status)")
        }
    }

    private fun enc(value: String): String = URLEncoder.encode(value, StandardCharsets.UTF_8.name())
    private fun seg(value: String): String = Uri.encode(value)
}
