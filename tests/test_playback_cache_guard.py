...
def test_large_movie_moves_to_external_before_internal_reserve_is_consumed(tmp_path, monkeypatch):
    primary = tmp_path / "internal" / ".nomad_cache" / "hls"
    external = tmp_path / "usb"
    ...
    def fake_volume(path, *, external):
        path = Path(path)
        if external:
            return _volume(path, external=True, free_mb=900, total_mb=1024)
        # 400 MB free looks healthy by percentage, but after keeping a 20%
        # reserve only ~195 MB is safe cache headroom.
        return _volume(path, external=False, free_mb=400, total_mb=1024)

    monkeypatch.setattr(cache_storage, "_volume", fake_volume)
    selected = cache_storage.choose_cache_root(
        primary,
...
def test_hls_guard_converts_full_disk_into_playback_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        cache_guard,
        "choose_cache_root",
        lambda _root, **_kwargs: (_ for _ in ()).throw(cache_storage.CacheStorageError("disk full safely")),
    )
...
def test_local_source_size_is_used_as_hls_cache_preflight(tmp_path):
    movie = tmp_path / "movie.mkv"
    movie.write_bytes(b"x" * 1024)
    manager = SimpleNamespace()
    assert cache_guard._expected_session_bytes(manager, {"source_path": str(movie)}) == 1024
