...
def test_generated_fmp4_remux_keeps_safe_h264_aac():
    ...
    plan = planner.plan(MediaProbe(container="matroska", video_codec="h264", audio_codec="aac"), iphone_like_caps())
...
def test_generated_fmp4_converts_mp3_audio_even_if_phone_supports_mp3_elsewhere():
    planner = BrowserPlaybackPlanner()
    plan = planner.plan(MediaProbe(container="matroska", video_codec="h264", audio_codec="mp3"), iphone_like_caps())
...
def test_generated_fmp4_converts_unsafe_video_codec_to_h264():
    caps = ClientCapabilities.from_values(containers=["mp4", "webm"], video_codecs=["vp9", "h264"], audio_codecs=["aac"])
    planner = BrowserPlaybackPlanner()
    plan = planner.plan(MediaProbe(container="matroska", video_codec="vp9", audio_codec="aac"), caps)
...
def test_high10_h264_does_not_direct_play_on_iphone():
    planner = BrowserPlaybackPlanner()
    plan = planner.plan(MediaProbe(container="mp4", video_codec="h264", audio_codec="aac", video_profile="High 10", pixel_format="yuv420p10le", codec_tag="avc1"), iphone_like_caps())
    assert plan.mode == PlaybackMode.TRANSCODE_VIDEO
    assert plan.target_video_codec == "h264"
...
def test_unknown_hevc_sample_entry_is_retagged():
    planner = BrowserPlaybackPlanner()
    plan = planner.plan(MediaProbe(container="mp4", video_codec="hevc", audio_codec="aac", pixel_format="yuv420p10le", codec_tag="hevx"), iphone_like_caps())
    assert plan.mode == PlaybackMode.REMUX
    assert plan.target_container == "mp4"
...
def test_ffmpeg_openmax_is_preferred_before_software_when_available():
    candidates = video_encoder_candidates("h264", available={"h264_omx", "libx264"}, policy="auto")
...
def test_sun60iw2_prefers_omx_over_generic_v4l2_when_both_exist():
    candidates = video_encoder_candidates("h264", available={"h264_v4l2m2m", "h264_omx", "libx264"}, policy="auto", model="sun60iw2")
    assert candidates == ["h264_omx", "libx264"]


def test_a733_gstreamer_worker_command_sets_vendor_decoder_rank(tmp_path):
    cmd = build_a733_hls_command(source_path="/media/movie.mkv", output_dir=tmp_path, start_position=12.5, width=1280, height=720, max_bitrate=4_000_000)
    joined = " ".join(cmd)
...
def test_a733_backend_requires_validated_omx_and_default_tracks(monkeypatch):
    ...
    monkeypatch.setattr("app.services.playback.gstreamer_a733.a733_gstreamer_status", lambda: {"usable": True})
    assert a733_backend_allowed(target_video_codec="h264", audio_stream_index=None, subtitle_stream_index=None) is True
    assert a733_backend_allowed(target_video_codec="hevc", audio_stream_index=None, subtitle_stream_index=None) is False
    assert a733_backend_allowed(target_video_codec="h264", audio_stream_index=2, subtitle_stream_index=None) is False
    assert a733_backend_allowed(target_video_codec="h264", audio_stream_index=None, subtitle_stream_index=4) is False


def test_a733_backend_can_be_disabled(monkeypatch):
    monkeypatch.setenv("NOMAD_A733_GSTREAMER", "off")
    monkeypatch.setattr("app.services.playback.gstreamer_a733.a733_gstreamer_status", lambda: {"usable": True})
    assert a733_backend_allowed(target_video_codec="h264", audio_stream_index=None, subtitle_stream_index=None) is False
...
def test_low_memory_remux_defaults_to_two_x_readrate(monkeypatch):
    ...
    monkeypatch.setattr("app.services.playback.hls.psutil.virtual_memory", lambda: SimpleNamespace(total=1024 * 1024 * 1024))
...
def test_remux_hls_command_uses_event_playlist_and_readrate(tmp_path):
    cmd = build_hls_command(source_path="movie.mkv", output_dir=Path(tmp_path), mode="remux", target_video_codec=None, target_audio_codec=None, source_video_codec="h264", input_readrate=2.0, readrate_supported=True)
...
def test_one_gib_device_uses_sequential_quality_handover(monkeypatch):
    ...
    monkeypatch.setattr("app.routers.playback_quality.psutil.virtual_memory", lambda: SimpleNamespace(total=1024 * 1024 * 1024))
