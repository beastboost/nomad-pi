...
def guess_title_year(name: str):
    ...
    # 0. Fast Path: If it already looks like "Title (Year)", trust it!
    # This prevents renaming files that are already perfect.
    # e.g. "Ant-Man (2015)" -> "Ant-Man", "2015"
    clean_match = re.match(r"^(.+?)\s+\((19\d{2}|20\d{2})\)$", s)
    if clean_match:
        return clean_match.group(1).strip(), clean_match.group(2)

    spam_prefix_patterns = [
        r"(?i)^\s*[\[\(\{]?\s*(?:www[\W_]*\.)?\s*(?:uindex|unidex)[\W_]*\.(?:org|com|net)[\W_]*[\]\)\}]?\s*",
        r"(?i)^\s*[\[\(\{]?\s*(?:www[\W_]*)?\s*(?:uindex|unidex)[\W_]*(?:org|com|net)[\W_]*[\]\)\}]?\s*",
        r"(?i)^\s*[\[\(\{]?\s*(?:www[\W_]*\.)?\s*(?:psarips|rarbg|galaxyrog|galaxyrg)[\W_]*\.(?:org|com|net)[\W_]*[\]\)\}]?\s*",
    ]
    for pat in spam_prefix_patterns:
        s = re.sub(pat, "", s).strip()
    
    # Remove website-like patterns anywhere in string
    s = re.sub(r"(?i)\b(?:www[\W_]*\.)?(?:uindex|unidex|psarips|rarbg|galaxyrog|galaxyrg)[\W_]*\.(?:org|com|net)\b", " ", s)
    s = re.sub(r"(?i)\b(?:www[\W_]*)?(?:uindex|unidex|psarips|rarbg|galaxyrog|galaxyrg)[\W_]*(?:org|com|net)\b", " ", s)
    ...
    tags = [
        ...
        r'yify', r'yts', r'rarbg', r'rargb', r'ettv', r'psa', r'psarips', r'tgx', r'qxr', r'utp', r'vxt',
        ...
        r'uindex', r'unidex', r'galaxyrog', r'galaxyrg', r'galaxyrg265', r'vostfr'
    ]
    ...
    # Modified to allow start of string and more flexible boundaries
    m = re.search(r'(?:^|[\s\.\(\[_\-])(19\d{2}|20\d{2})(?:$|[\s\.\)\]_\-])', s)
    
    if m:
        year = m.group(1)
        # Title is everything before the year
        # Be careful if the year is at the start, title might be empty (which is handled later)
        if m.start() == 0:
             # If year is at start, maybe title is AFTER year? e.g. "2015 Ant Man"
             # But usually it's "Title 2015". Let's stick to "before year" logic first.
             # If year is at 0, title_part is empty.
             # Check if there is text AFTER the year.
             remainder = s[m.end():].strip()
             if remainder:
                 title_part = remainder
             else:
                 title_part = s # Fallback, keep year in title if nothing else
        else:
    ...
    if len(title) < 2:
        title = os.path.splitext(os.path.basename(name or ""))[0].strip()
...
def extract_archive_to_dir(archive_path: str, out_dir: str):
    ...
    if not attempts:
        error_msg = (
            ...
            "1️⃣ SSH into your system\n"
            "2️⃣ Run: sudo apt-get update && sudo apt-get install -y 7zip unrar unar libarchive-tools\n"
            "3️⃣ Restart Nomad Pi service\n\n"
            "📝 Note: These packages should have been installed during updates. "
...
def delete_media(path: str, background_tasks: BackgroundTasks, admin: dict = Depends(get_current_admin)):
    fs_path = safe_fs_path_from_web_path(path)
